...
Load the state-level covid count data that we saved as GeoPackage. If you are a regular GIS user you might like to use GeoPackage more often and get rid of arcane shapefile, if you still use it. 
...
```
You would perhaps like to see the parititon plot we created earlier and the map above together. You can try 'cowplot' or 'gridExtra'. If you want to format axix lables see this (https://stackoverflow.com/questions/11610377/how-do-i-change-the-formatting-of-numbers-on-an-axis-with-ggplot). You must be wondering that with web-based mapping we can easily provide more information. We will try that in later, for now let us pursue the static map option a bit further. 
